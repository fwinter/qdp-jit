...
#ifndef PETE_PETE_FURJEDES_H
#define PETE_PETE_FURJEDES_H
...
//
// CLASS NAME
//   Furjedes<Expr, FTag, CTag>
//   furjedes(Expr& e, FTag& f, CTag& c)
//
// Expr is the type of the expression tree.
// FTag is the type of the leaf tag.
// CTag is the type of the combiner tag.
//
// Furjedes<Expr,FTag,CTag>::apply(Expr &e,FTag& f,CTag& c) is a function
...
// typename Furjedes<Expr,FTag,CTag>::Type_t
// the function furjedesTag(Expr& e,FTag& f,CTag& c) duplicates the action
// of Furjedes::apply and is provided for convenience.  (You don't have to
// type the template arguments.)
//
// This generic Furjedes functor differs from the original
// PETE furjedes in 3 ways:
//  1) The user should not specialize Furjedes.
...
template<class Expr, class FTag, class CTag>
inline typename FurJedes<Expr,FTag,CTag>::Type_t
furJedes(const Expr &e, const FTag &f, const CTag &c)
{
  return FurJedes<Expr, FTag, CTag>::apply(e, f, c);
}

template<class Op, class A, class FTag, class CTag>
struct FurJedes<UnaryNode<Op, A>, FTag, CTag>
{
  typedef typename FurJedes<A, FTag, CTag>::Type_t TypeA_t;
    ...
    const CTag &c) 
  {
    return Combine1<TypeA_t, Op, CTag>::
      combine(FurJedes<A, FTag, CTag>::apply(expr.child(), f, c),
              expr.operation(), c);
  }
};

template<class Op, class A, class B, class FTag, class CTag>
struct FurJedes<BinaryNode<Op, A, B>, FTag, CTag >
{
  typedef typename FurJedes<A, FTag, CTag>::Type_t TypeA_t;
  typedef typename FurJedes<B, FTag, CTag>::Type_t TypeB_t;
	       ...
	       const CTag &c) 
  {
    return Combine2<TypeA_t, TypeB_t, Op, CTag>::
      combine(FurJedes<A, FTag, CTag>::apply(expr.left(), f, c),
              FurJedes<B, FTag, CTag>::apply(expr.right(), f, c),
	      expr.operation(), c);
  }
};

template<class Op, class A, class B, class C, class FTag, class CTag>
struct FurJedes<TrinaryNode<Op, A, B, C>, FTag, CTag >
{
  typedef typename FurJedes<A, FTag, CTag>::Type_t TypeA_t;
  typedef typename FurJedes<B, FTag, CTag>::Type_t TypeB_t;
  typedef typename FurJedes<C, FTag, CTag>::Type_t TypeC_t;
	       ...
	       const CTag &c) 
  {
    return Combine3<TypeA_t, TypeB_t, TypeC_t, Op, CTag>::
      combine(FurJedes<A, FTag, CTag>::apply(expr.left(), f, c),
	      FurJedes<B, FTag, CTag>::apply(expr.middle(), f, c),
	      FurJedes<C, FTag, CTag>::apply(expr.right(), f, c),
	      expr.operation(), c);
  }
};
...
struct FurJedes<Expression<T>, FTag, CTag>
{
  typedef typename FurJedes<T, FTag, CTag>::Type_t Type_t;
  inline static
  Type_t apply(const Expression<T> &expr, const FTag &f, 
	       const CTag &c) 
  {
    return FurJedes<T, FTag, CTag>::apply(expr.expression(), f, c);
  }
};
...
template<class T, class FTag, class CTag>
struct FurJedes<Reference<T>, FTag, CTag>
{
  typedef typename FurJedes<T, FTag, CTag>::Type_t Type_t;
  inline static
  Type_t apply(const Reference<T> &ref, const FTag &f, 
	       const CTag &c) 
  {
    return FurJedes<T, FTag, CTag>::apply(ref.reference(), f, c);
  }
};

#endif // PETE_PETE_FURJEDES_H

// ACL:rcsinfo
// ----------------------------------------------------------------------
// $RCSfile: FurJedes.h,v $   $Author: edwards $
